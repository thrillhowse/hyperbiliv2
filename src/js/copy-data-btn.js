export const addCopyBtnEL = (data) => {
  const copyBtn = document.querySelector('#copy-bili-data');
  copyBtn.addEventListener('click', async (event) => {
    ...
    let str = ``;
    if (data.patient.dob) {
      str += `Date of Birth: ${data.patient.dob}<br>`;
    }
    if (data.patient.hob) {
      str += `Hour of Birth: ${data.patient.hob}:00<br>`;
    }
...
/**
 * Restrict the styling when users select/copy the info from the table
 * note: this is in response to users not always using the Copy button and just copying the text
 * which leads to the table styles coming along and looking weird when pasted into EMRs and such
 */
document
  .querySelector('.summary-tables-container')
